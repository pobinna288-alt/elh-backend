  ...
  if (followerTrustScore >= TRUST_THRESHOLDS.MIN_INFLUENCE_TRUST) {
    ...
    if (stats.total_trust_boost_received < TRUST_BOOST.TOTAL_CAP) {
      ...
      if (trustBoostAmount > 0) {
        ...
        // Update seller trust score via trustScoreService (single source of truth)
        const previousScore = seller.trust_score || 50;
        const updateResult = updateTrustScore(seller, trustBoostAmount, 'follower_boost', {
          description: `Follower trust score: ${followerTrustScore}`,
          source: 'system'
        });
        const newScore = updateResult.newScore;
        
        // Log to local audit trail (used by /trust-log endpoint)
        logTrustScoreChange({
          userId: sellerId,
          previousScore,
          newScore,
