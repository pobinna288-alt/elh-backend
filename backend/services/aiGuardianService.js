...
// Anthropic Claude 3.5 Sonnet (Enterprise only)
...
try {
  ...
  console.warn("[AI Guardian] Anthropic initialization skipped:", err.message);
}

// Google Gemini 1.5 Pro (Enterprise only)
...
try {
  ...
  console.warn("[AI Guardian] Gemini initialization skipped:", err.message);
}
...
const SUBSCRIPTION_FEATURES = {
  ...
  enterprise: {
    // AI Model Configuration - Ensemble of 3 models
    aiModels: ["gpt-4o", "claude-3.5-sonnet", "gemini-1.5-pro"],
    useMultiModel: true,
    aiModelDescription: "Ensemble reasoning from three most advanced AI models",
...
// ─── Enterprise Multi-Model Analysis ────────────────────────────────────────

/**
 * Model 1 (GPT-4o): Demand + Trend Detection
 */
...
/**
 * Model 2 (Claude 3.5 Sonnet): Audience Fit + Campaign Structure
 */
async function analyzeWithClaude(campaignData) {
  if (!anthropicClient) {
    return { error: "Anthropic not available", fallback: true };
  }
  
  try {
    const prompt = `You are an AI audience analyst. Analyze this campaign for audience fit and structure.

Product: ${campaignData.product_name}
Description: ${campaignData.product_description}
Target Audience: ${campaignData.target_audience}
Target Age Range: ${campaignData.target_age_range || "Not specified"}
Target Country: ${campaignData.target_country}
...
Return JSON with:
{
  ...
  "audienceAlignmentIssues": ["string array"],
  "campaignStructureScore": number 0-100,
  "structureRecommendations": ["string array"],
  "platformFit": "Excellent" | "Good" | "Fair" | "Poor"
    ...
    return jsonMatch ? JSON.parse(jsonMatch[0]) : { error: "Parse failed", fallback: true };
  } catch (error) {
    console.error("[AI Guardian Claude]", error.message);
    return { error: error.message, fallback: true };
  }
}

/**
 * Model 3 (Gemini 1.5 Pro): Pricing + Market Competitiveness
 */
async function analyzeWithGemini(campaignData) {
  if (!geminiClient) {
  ...
  try {
    ...
    const prompt = `You are an AI pricing analyst. Analyze this campaign for pricing and market competitiveness.
...
Return ONLY valid JSON with:
{
  ...
  "pricingRisks": ["string array"]
    ...
    console.error("[AI Guardian Gemini]", error.message);
    return { error: error.message, fallback: true };
  }
}

/**
 * Enterprise aggregation layer - combines outputs from all 3 models
 */
function aggregateMultiModelResults(gpt4oResult, claudeResult, geminiResult, localAnalysis) {
  // Start with local analysis as baseline
  let aggregatedScore = localAnalysis.campaignSafetyScore;
  let confidenceScore = 70;
  let modelContributions = [];
  
  // Aggregate GPT-4o demand analysis
  if (!gpt4oResult.fallback && gpt4oResult.demandSignal) {
    const demandBoost = {
      "Very High": 10, "High": 5, "Moderate": 0, "Low": -5, "Declining": -10
    }[gpt4oResult.demandSignal] || 0;
    aggregatedScore = Math.round((aggregatedScore + demandBoost) * 0.95);
    confidenceScore += gpt4oResult.demandConfidence ? (gpt4oResult.demandConfidence / 10) : 5;
    modelContributions.push({ model: "GPT-4o", task: "Demand Analysis", status: "Complete" });
  } else {
    modelContributions.push({ model: "GPT-4o", task: "Demand Analysis", status: "Fallback" });
  }
  
  // Aggregate Claude audience analysis
  if (!claudeResult.fallback && claudeResult.audienceFitScore) {
    const audienceWeight = (claudeResult.audienceFitScore - 50) / 5;
    aggregatedScore = Math.round(aggregatedScore + audienceWeight);
    confidenceScore += 8;
    modelContributions.push({ model: "Claude 3.5 Sonnet", task: "Audience Analysis", status: "Complete" });
  } else {
    modelContributions.push({ model: "Claude 3.5 Sonnet", task: "Audience Analysis", status: "Fallback" });
  }
  
  // Aggregate Gemini pricing analysis
  if (!geminiResult.fallback && geminiResult.marketPositionScore) {
    const pricingWeight = (geminiResult.marketPositionScore - 50) / 5;
    aggregatedScore = Math.round(aggregatedScore + pricingWeight);
    confidenceScore += 7;
    modelContributions.push({ model: "Gemini 1.5 Pro", task: "Pricing Analysis", status: "Complete" });
  } else {
    modelContributions.push({ model: "Gemini 1.5 Pro", task: "Pricing Analysis", status: "Fallback" });
  }
  
  // Normalize scores
  aggregatedScore = Math.min(100, Math.max(0, aggregatedScore));
  confidenceScore = Math.min(100, Math.max(50, confidenceScore));
  
  // Determine final risk level
  let riskLevel;
  if (aggregatedScore >= 80) riskLevel = "Low";
  else if (aggregatedScore >= 60) riskLevel = "Moderate";
  else if (aggregatedScore >= 40) riskLevel = "High";
  else riskLevel = "Critical";
  
  return {
    finalCampaignSafetyScore: aggregatedScore,
    finalRiskLevel: riskLevel,
    confidenceScore: confidenceScore,
    confidencePercentage: `${confidenceScore}%`,
    modelContributions: modelContributions,
    gpt4oInsights: gpt4oResult.fallback ? null : gpt4oResult,
    claudeInsights: claudeResult.fallback ? null : claudeResult,
    geminiInsights: geminiResult.fallback ? null : geminiResult
  };
}
 ...
 * Uses AI models based on subscription tier for Enterprise ensemble analysis
 */
async function generateAIEnhancedGuardianAnalysis(campaignData) {
  ...
  // For Enterprise tier, use multi-model ensemble analysis
  if (features.useMultiModel) {
    try {
      // Run all 3 models in parallel for ensemble reasoning
      const [gpt4oResult, claudeResult, geminiResult] = await Promise.all([
        analyzeWithGPT4o(campaignData),
        analyzeWithClaude(campaignData),
        analyzeWithGemini(campaignData)
      ]);
      
      // Aggregate results from ensemble
      const aggregated = aggregateMultiModelResults(gpt4oResult, claudeResult, geminiResult, {
        campaignSafetyScore: baseAnalysis.safety_score
      });
      
      // Update safety score with ensemble result but keep fixed confidence score
      return {
        ...baseAnalysis,
        safety_score: aggregated.finalCampaignSafetyScore,
        risk_level: aggregated.finalRiskLevel,
        // confidence_score stays fixed at 91 for Enterprise
        _ensemble_metadata: {
          models_used: features.aiModels,
          model_contributions: aggregated.modelContributions
        }
      };
    } catch (error) {
      console.error("[AI Guardian Enterprise Ensemble]", error.message);
      // Return base analysis on error
      return baseAnalysis;
    }
  }
  
  // For Pro/Hot tiers, enhance with single model if available
  if (openaiClient) {
    try {
...
function getFeatureComparison() {
  return {
    ...
    tiers: {
      ...
      enterprise: {
        name: "Enterprise",
        aiModel: "Ensemble reasoning from three most advanced AI models",
        confidenceScore: 91,
        limits: {
          topTargetMarkets: 6,
