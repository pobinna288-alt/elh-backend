...
function activateUserFeatures({ database, transaction, createNotification }) {
  ...
  if (subStatus.active) {
    console.log("Subscription blocked: active plan exists for user:", transaction.userId);
    return {
      success: false,
...
function createPaymentsService({ database, createNotification, onPaymentActivated }) {
  ...
  async function initializePayment(payload = {}) {
    ...
    if (subStatus.active) {
      ...
      const blocked = buildActiveSubscriptionResponse(subStatus);
